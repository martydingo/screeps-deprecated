    ...
    "feederBots": {
                    ...
                    if(creep.store[RESOURCE_ENERGY] < 50) {
                        ...
                        if(creep.transfer(variables.spawns.E26N37.primarySpawn(), RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                            ...
                            creep.say("📦")
                                ...
                                creep.moveTo(variables.structures.extensions.E26N37.notFull()[0], {visualizePathStyle: {stroke: '#ffffff'}})
                            }
        ...
        "E27N38":{
            "primarySpawn": {
                feedSpawnExtensions: function(creep) {
                    ...
                    if(creep.store[RESOURCE_ENERGY] < 50) {
                        ...
                        if(creep.transfer(variables.spawns.E27N38.primarySpawn(), RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                            creep.moveTo(variables.spawns.E27N38.primarySpawn(), {visualizePathStyle: {stroke: '#ffffff'}})
                            creep.say("📦")
                        } 
                    } 
                    else if(creep.transfer(variables.structures.extensions.E27N38.notFull()[0],RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                                creep.moveTo(variables.structures.extensions.E27N38.notFull()[0], {visualizePathStyle: {stroke: '#ffffff'}})
                            }
                ...
                run: function(creep){
                        bots.feederBot.E27N38.primarySpawn.feedSpawnExtensions(creep)
            ...
            run: function(creep) {
                ...
                else 
                {
                    ...
                    if(creep.memory.shouldBuild) {
                        var targets = creep.room.find(FIND_CONSTRUCTION_SITES);
                        var targetsRepair = creep.room.find(FIND_STRUCTURES, { filter: object => object.structureType == STRUCTURE_CONTAINER})
                        if(targets.length) {
        ...
        "E27N38": {
            run: function(creep) {
                ...
                else 
                {
                    ...
                    if(creep.memory.shouldBuild) {
                        var targets = creep.room.find(FIND_CONSTRUCTION_SITES);
                        var targetsRepair = creep.room.find(FIND_STRUCTURES, { filter: object => object.structureType == STRUCTURE_CONTAINER}).sort(function(a,b){ return a.hits - b.hits})
                        if(targets.length) {
            ...
            run: function(creep){
                if(configuration.bots.claimBots.E27N38.claimTarget != ""){
                    ...
                    if(claimResult == ERR_NOT_IN_RANGE){
                        ...
                        creep.moveTo(new RoomPosition(46,4,'E27N38'), {visualizePathStyle: {stroke: '#ffffff'}})
                    }
                }
            }
        },
